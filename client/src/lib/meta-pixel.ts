...
async function sendCAPIEvent(eventName: string, eventData: CAPIEventData, eventId: string) {
  ...
  console.log(`[CAPI Server] Sending ${eventName} (event_id: ${eventId})${testEventCode ? ` [TEST: ${testEventCode}]` : ''} — payload:`, JSON.parse(JSON.stringify(payload)));
  try {
    ...
    console.log(`[CAPI Server] ${eventName} (event_id: ${eventId}) — response:`, result);
  } catch (err) {
